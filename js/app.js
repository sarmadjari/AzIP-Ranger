...
(function () {
  "use strict";
  const C = window.LZ_CIDR, E = window.LZ_ENGINE;
    ...
    if (plan.nva && plan.nva.tiers.length) {
      ...
      plan.nva.tiers.forEach(t => {
        t.groups.forEach((g, gi) => {
          ...
          if (g.vmss) {
            rows2 += `<tr><td class="mono">${esc(g.vmssName)}</td><td class="mono">dynamic <span class="muted">${t.extSubnet}</span></td><td class="mono">dynamic <span class="muted">${t.intSubnet}</span></td><td class="mono">dynamic</td><td class="mono">${g.hop.ip} <span class="muted">via VMSS profile</span></td></tr>`;
          } else {
            rows2 += g.instances.map(x =>
              `<tr><td class="mono">${esc(x.name)}</td><td class="mono">${x.ext} <span class="muted">${t.extSubnet}</span></td><td class="mono">${x.int} <span class="muted">${t.intSubnet}</span></td><td class="mono">${x.mgmt || "—"}</td><td class="mono">${x.loopbacks.join(", ") || "—"}</td></tr>`).join("");
          }
        });
      });
  ...
  function buildMarkdown() {
    ...
    lines.push(`> Generated ${new Date().toISOString().slice(0, 10)} · based on IP Plan v5.1 + Network Design Guide v1.0 · region: ${s.region}\n`);
    ...
    if (plan.nva && plan.nva.tiers.length) {
      ...
      plan.nva.tiers.forEach(t => {
        if (t.groups.length > 1) {
          lines.push(`**${t.label} tier chain**: ${t.groups.map((g, i) => `${i + 1}. ${g.display} (${g.hop.ip})`).join(" → ")} — route tables target only the entry hop; each group forwards to the next and SNATs to its internal NICs.\n`);
        }
        t.groups.forEach((g, gi) => {
          const pos = t.groups.length > 1 ? ` (chain ${gi === 0 ? "entry" : "hop " + (gi + 1)})` : "";
          if (g.vmss) {
            lines.push(`**${g.display} — ${t.label}${pos}** — VMSS Flex (${g.vmssName}), autoscale ${g.scale.min}–${g.scale.max} behind ${g.ilbName} (VIP ${g.hop.ip}). Instance NICs are dynamic in ${t.extSubnet} / ${t.intSubnet}.\n`);
          } else {
            ...
            lines.push(mdTable(["NVA", "External NIC", "Internal NIC", "Mgmt NIC", "Loopback VIP"],
              g.instances.map(x => [x.name, `${x.ext} (${t.extSubnet})`, `${x.int} (${t.intSubnet})`, x.mgmt || "—", x.loopbacks.join(", ") || "—"])));
          }
        });
      });
    }
  ...
  function csvSubnets() {
    ...
    plan.pools.forEach(p => p.allocs.forEach(a => {
      ...
      (plan.templates[a.template] || []).forEach(t => {
        ...
        const named = t.tier.startsWith("(");
        rows.push(["Spoke", a.name, a.env, named ? t.tier : `snet-${t.tier}-${a.nameBase}`, C.cidr(a.base + t.off, t.prefix), C.usable(t.prefix), t.purpose, a.rtName, named ? "per workload" : `nsg-${t.tier}-${a.nameBase}`, "", "Active"]);
      });
    }));
  ...
  function csvAll() {
    return [
      "### VNETS", csvVnets(), "",
      "### SUBNETS", csvSubnets(), "",
      "### ROUTE TABLES", csvRoutes(), "",
      "### NSG RULES", csvNsg(),
    ].join("\r\n");
  }

  function designJson() {
    return JSON.stringify({ tool: "azip-ranger", version: 2, savedAt: new Date().toISOString(), state }, null, 2);
  }
