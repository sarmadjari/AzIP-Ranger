...
## Production Reference Architecture — v5.2 (Final)

> **Document Status**: Production-Ready
> **Version**: v5.2 — supersedes v5.1; applies the deep-scan fixes **F1–F6** (`Test/VALIDATION-REPORT.md`, adjudicated against live Microsoft Learn 2026-06-12): exact-prefix routing vs peering LPM, the fabric-routed chain cascade, and four NSG corrections (§19.7)
> **Classification**: Enterprise Network Architecture Blueprint  
> **Scope**: Hub-and-spoke topology with dual-tier NVA security (North-South + East-West), extended in v5.1 with **NVA group chains, VMSS-based appliance groups, chained Azure Firewall, and catalog-driven CAF naming** (§20–§25)  
> **Companion tool**: **AzIP-Ranger** (`index.html`) implements and validates every rule in §20–§25 with an automated test suite (`tests/run.js`, including an effective-route LPM simulator); all selectable values live in `js/config.js` (`AZIP_CONFIG`)  
> **Verified Against**: **Microsoft Learn** and **Azure Cloud Adoption Framework / Azure Landing Zone guidance** — six CAF pages fetched and applied 2026-06-11 (v5.0 core), plus the HA-NVA, Azure Firewall SNAT, Gateway Load Balancer, CAF resource naming/abbreviations, and Azure subscription-limits guidance for the v5.1 extensions (§26)

> ### What's new in v5.2 (deep-scan fixes)
> A full validation of generated designs against live Microsoft Learn surfaced two critical routing defects and four NSG gaps (IDs **F1–F6**, `Test/VALIDATION-REPORT.md`). v5.2 fixes all of them — see **§19.7** for the changelog and root-cause attribution. Headlines:
>
> 1. **F1 (Critical, §7–§8)** — Azure selects routes by **longest-prefix-match across all sources**; UDR-beats-system applies only at *identical* prefixes ([route selection](https://learn.microsoft.com/azure/virtual-network/virtual-networks-udr-overview#how-azure-selects-routes-for-traffic-routing)). The former `To-Hub 10.0.0.0/16` and `To-Spokes 10.4.0.0/14` summaries silently lost to the more-specific peering system routes: spoke↔hub and platform→spoke traffic bypassed inspection and **on-prem↔spoke flows broke outright** (forward leg bypassed the EW NVA, return leg hit it stateful → dropped). All steering routes now use **exact peered-VNet prefixes** (hub /19s in spoke RTs; one route per spoke in hub-side RTs), with direct-return exemptions for Bastion and every NVA internal subnet.
> 2. **F2 (Critical, §20–§21)** — the v5.1 same-subnet chain ("appliance forwards to the next VIP") has no Azure fabric mechanism: Azure forwards on destination IP per effective routes and ignores guest-OS next hops toward a same-subnet VIP. Chains are now a **fabric-routed UDR cascade**: each chained group ≥ 2 owns a dedicated internal subnet (plus one forward subnet for the EW/single tier's group 1), and per-subnet route tables steer segment → segment — Azure Firewall slots included. Depth > 3 triggers a warning (Gateway Load Balancer recommended for N-S insertion).
> 3. **F3–F6 (§9, §11.3)** — spoke PE NSGs now allow the spoke's own workloads; chained tiers allow hop-to-hop segments; the Key Vault PE subnet got its missing 443 allow; the dead `data→app 1433` rule is gone; NSG-Management is scoped to the declared hub prefixes; §6.5's SNAT scope now explicitly covers PE-hosting subnets.

> ### What's new in v5.1
> v5.1 folds the former *Network Design Addendum* into this document as six new sections. **Nothing in the v5.0 core (§1–§19) is invalidated** — the default single-group design is reproduced exactly (all anchors — `10.0.3.100`, `10.0.5.100`, the three hub /19s, the T-shirt pools — unchanged). The additions are opt-in extensions:
>
> | New section | Capability | Extends |
> |---|---|---|
> | **§20 NVA Group Chains** | Ordered, chained NVA groups per inspection tier (entry-hop UDR + VIP ladder + per-group SNAT), with a precise per-/24 address law and a "when not to chain" decision record | §5–§6 |
> | **§21 Chained Azure Firewall** | Azure Firewall as a deployable chain element on the NS / EW / single tier (any slot), all keeping the NVA tiers intact | §1.2.3, §6.5, §12.3 |
> | **§22 VMSS-based NVAs** | VMSS Flexible appliance groups (dynamic NICs, stable VIP, scale floor ≥ 2) behind the group ILB | §6.6 |
> | **§23 Catalog-driven Spokes & CAF Naming** | Free-form spoke catalog (name/env/size as labels), Reference vs Auto allocation modes, and the full CAF naming convention | §4 |
> | **§24 Operational Gating Rules** | Tool behaviour for hybrid connectivity, ER+VPN precedence, and the Region-2 reservation | §1.1, §2.1 |
> | **§25 Updated Capacity Guardrails** | Per-tier static-NIC ceiling (96), chained-group ceiling (155), and the exact 251-per-/24 check | §15.4 |
...
| Zero Trust | All inter-zone traffic inspected by NVAs; no direct spoke-to-spoke or spoke-to-internet. Two documented, deliberate exceptions: intra-hub Shared Services → Hub PE (system route, §11.3) and on-prem → Hub PE (direct, §11.3) — compensating controls are the PE NSG + `privateEndpointNetworkPolicies = Enabled`. |
...
|---|---|
| Resources in one (or a few) Azure regions; no full-mesh global transit needed | ✅ Single region today; two-region template defined (§2.1) |
| Fewer than ~30 IPsec site-to-site tunnels per region | ✅ ExpressRoute is the primary hybrid path; VPN is backup only |
| Full control and granularity over routing policy | ✅ The entire design is granular UDR + NSG control (§7–§9) |
...
| **Identity** (Platform LZ) | The AD DS Domain Controllers (Subnet-DomainControllers `10.0.33.0/24`). CAF permits DCs either in the central hub ("If necessary, also deploy Active Directory domain controllers and DNS servers" — traditional-topology page) or in a dedicated Identity-subscription VNet peered to the hub. **This design keeps them in the hub VNet** for DNS/latency simplicity; if your operating model requires Identity-subscription ownership, carve a peered Identity VNet from the platform-growth space (§2) and move `10.0.33.0/24` semantics there — routes/NSGs in this document transfer unchanged. |
| **Management subscription** (Platform LZ) | The PaaS management estate (Log Analytics, Azure Automation accounts, Recovery Services vaults, Update Manager). Their **network touchpoints** — private endpoints and any management VMs — live in the hub's Management /19 subnets (§3.3), which physically sit in the Connectivity subscription because they share the Hub VNet. |
| **Corp management group** (Application LZs) | Every Prod/Dev/Test **spoke VNet** in this plan: internal-facing, hub-routed, no direct Internet exposure. One application landing zone (subscription) per workload **per environment**, vended with a spoke from the §4 pools. |
| **Online management group** (Application LZs) | Public-facing workloads. Pattern: an Online spoke peers to the hub like any Corp spoke for east-west/egress, but its **inbound** L7 path is its own **Application Gateway (with WAF) deployed inside the spoke** — per CAF, L7 inbound NVAs are *not* shared hub services (§5 inbound note). Azure Front Door fronts multi-region/global entry. |
| **Policy inheritance** | Spokes inherit ALZ policy assignments from their MG (Corp/Online); §12.7 lists the network guardrails this document expects as policy-as-code. |

#### 1.2.3 Design-principles alignment & deviation register

| ALZ design principle | How this design implements it |
|---|---|
| **Subscription democratization** | Spokes are vended per workload per environment from pre-planned, non-overlapping pools using the **T-shirt catalog** (§4.1.1) and IPAM API integration (§4.4) — application teams self-serve address space without platform-team bottlenecks. |
| **Policy-driven governance** | Network invariants are enforced as Azure Policy + Terraform plan gates, not tribal knowledge: §12.7 catalog + the fact-check report's Section L checks. |
...
| **Application-centric service model** | One spoke = one application landing zone; environments (Dev/Test/Prod) are separated by **subscription + spoke pool**, identical NSG/RT semantics across environments (§4.2/§4.2.1) so promotion testing exercises production security behaviour. |
| **Azure-native design alignment** | **Documented deviation**: third-party NVAs are used instead of Azure Firewall for NS/EW inspection (vendor feature requirements: TLS inspection depth, vendor SD-WAN integration, existing operational expertise). The native exit ramp is pre-provisioned — `AzureFirewallSubnet` + `AzureFirewallManagementSubnet` are reserved (§3.1) and §6.5/§12.3 already document Azure Firewall SNAT/UDR behaviour, so replacing NVAs with Azure Firewall Premium is a routing cut-over, not a re-design. |
...
| ├─ Platform Landing Zone | `10.0.0.0/16` | Hub VNet sections, shared services, management (§3) |
| ├─ Reserved — Platform growth | `10.1.0.0/16`, `10.2.0.0/15` | Held contiguous with the Platform /16 so platform expansion (second hub VNet, Identity VNet per §1.2.2, or hub address-space additions) summarises as `10.0.0.0/14` without renumbering. Do not assign to spokes. |
| └─ Application Landing Zones | `10.4.0.0/14` | Spoke pools — Prod, Dev, Test (§4) |
...
| Region 3+ | *(new block required)* | The /12 is fully consumed by two regions. Allocate the next region from a **new, non-overlapping** RFC 1918 block (e.g., `10.16.0.0/12`) reserved in IPAM (§4.4) before any Region-3 work begins. |
...
2. **Spoke route tables already comply** with the CAF cross-region inspection mitigation: `RT-Spoke-Workloads` has **BGP propagation disabled** (§8), so remote-region spoke prefixes never silently appear in spoke effective routes via ExpressRoute — cross-region reachability is granted only by explicit UDR (`10.12.0.0/14 → 10.0.5.100` in Region-1 spokes), keeping the firewall in path.
3. **More than two regions**: per CAF, prefer connecting every regional hub to the **same ExpressRoute circuits**, or re-evaluate **Virtual WAN** (§1.2.1 triggers) — global peering at >2 regions multiplies peering relationships and UDR complexity.
...
- **Address-space changes are online operations**: a VNet's address space can be extended after creation without outage; each existing peering then requires a **resync** ([update peering address space](https://learn.microsoft.com/en-us/azure/virtual-network/update-virtual-network-peering-address-space)). This is the designed growth mechanism for the Hub VNet (§3.0) — budget the resync into the change window for environments with hundreds of spoke peerings.
...
> |---|---|
> | `10.0.0.0/19` | Connectivity (§3.1) |
> | `10.0.32.0/19` | Shared Services (§3.2) |
> | `10.0.64.0/19` | Management (§3.3) |
>
> **24,576 addresses declared instead of 65,536** — and every subnet in §3.1–§3.3 falls inside these prefixes, so **nothing is renumbered** (verified computationally, all 21 subnets ⊂ the three /19s). The remaining `10.0.96.0–10.0.255.255` stays **plan-reserved, unassigned**: either a fourth `/19` prefix is *added to the Hub VNet online* when needed (address-space extension is a no-outage operation; each spoke peering then needs a resync — §2.2), or it seeds a separate platform VNet (e.g., the Identity VNet option in §1.2.2).
>
> **Side-effects, all assessed**: (a) the ER/VPN gateway now advertises **3 hub prefixes** to on-prem instead of 1 (budgeted in §15.4); (b) the `VirtualNetwork` NSG service tag for hub NICs narrows to the declared prefixes — no functional change, every rule in §9/§11 uses literal CIDRs; (c) `RT-Spoke-Workloads`' `To-Hub 10.0.0.0/16 → 10.0.5.100` route is **deliberately kept as the /16 supernet** — UDR prefixes need not match VNet space, and any packet to the unassigned 10.0.96.0+ range is steered to the EW NVA and dropped there (an inspected black-hole, preferable to falling through to the Internet default route).
...
> **v5.2 (F2) — chain-segment subnets**: when an inspection tier runs **≥ 2 chained NVA groups** (§20), each chained group i ≥ 2 receives a dedicated `/24` internal subnet (`Subnet-NS-Internal-i` / `Subnet-EW-Internal-i`), and a chained East-West/single tier adds one `Subnet-EW-Forward` (`Subnet-FW-Forward`) for group 1's forward NICs. They are allocated **after** `Subnet-PrivateEndpoints-Hub` (from `10.0.9.0/24` upward) so every v5.0 anchor above is unchanged. Single-group tiers (the default) create none of them.

> **v4.9 fix — no route table on Subnet-PrivateEndpoints-Hub**: earlier versions referenced an `RT-PrivateEndpoints` that was never defined anywhere in §7. None is needed: Private Endpoints originate no traffic of their own, and **PE return traffic bypasses UDRs** unless `privateEndpointNetworkPolicies` route-table enforcement plus a deliberate UDR pattern is engineered. Associating an empty route table would add an object with zero effect. NSG enforcement (§11.3) is retained via `privateEndpointNetworkPolicies = Enabled`.

### 3.2 Shared Services (`10.0.32.0/19`)

> **Important**: This design supports **Azure Private DNS Resolver (PaaS)**, which requires two dedicated delegated subnets. The subnets must be delegated to `Microsoft.Network/dnsResolvers` and cannot host any other resources. NSGs are **omitted** on the DNS Resolver delegated subnets by conservative design choice — current Microsoft docs do not explicitly prohibit them (see §3.2.1 and §12.5), but the resolver subnets carry only managed-service traffic and an NSG adds risk without benefit here.
...
|------|--------|-----------|------|----------|-----------------|
| **Small** | `/24` | 256 | Dev `10.6.0.0/16` and Test `10.7.0.0/16` | §4.2.1 | Sequential from the bottom of the environment's /16 |
| **Medium** (default for Production) | `/22` | 1,024 | Prod `10.4.0.0/15` | §4.2 | Sequential **bottom-up** from `10.4.0.0/22` |
| **Large** | `/20` | 4,096 | Prod `10.4.0.0/15` (same pool) | §4.2 applied per-/22 quarter, or workload-defined (AKS CNI, AVS, big data) | **Top-down** from `10.5.240.0/20`, descending — each Large consumes a **/20-aligned run of four contiguous /22 slots** |
...
> **v5.0 (CAF) — peering operations at scale**: (a) whenever the **Hub VNet address space changes** (a fourth /19 prefix is added per §3.0), every existing spoke peering requires a **resync** before the new prefix is reachable — schedule it as part of the change, it is online but per-peering; (b) at hundreds of spokes, manage peering creation and the §4.3 flag set through **Azure Virtual Network Manager** connectivity configurations (hub-and-spoke topology type) instead of per-spoke IaC peering resources — AVNM enforces the flag combinations centrally and remediates drift.
...
|---|---|
| Centralized inventory & overlap prevention | Deploy **[Azure IPAM](https://azure.github.io/ipam)** (open-source, Azure-native, Entra ID auth, REST API) in the Management subscription. Seed it with the §2 regional /13 template; it auto-discovers actual VNet utilization tenant-wide. |
| Request sizing | Application teams request by **T-shirt size** (§4.1.1) through the vending pipeline; the pipeline maps S/M/L → pool → next slot per the §4.1.1 allocation rules. |
...
| Structure | Organize IPAM blocks by **region → environment → workload archetype** mirroring §2/§4 exactly, so the inventory *is* this document's tables, live. |
| Decommissioning | Workload offboarding **releases the reservation** in the same pipeline run that destroys the spoke; freed /22 and /24 slots return to their pool's free list (the two-pointer Large/Medium scheme in §4.1.1 stays fragmentation-free because releases re-enter the correct end). |
...
Each NVA requires **2 NICs** (not 3):

| NVA Role | NIC 1 (External) | NIC 2 (Internal) |
|----------|------------------|------------------|
| North-South NVA | `Subnet-NS-External` (`10.0.2.x`) | `Subnet-NS-Internal` (`10.0.3.x`) |
| East-West NVA | `Subnet-EW-External` (`10.0.4.x`) | `Subnet-EW-Internal` (`10.0.5.x`) |
...
> **v4.9 fix**: `10.0.2.100` dropped from the NS loopback list — `ILB-NS-Inbound` was removed in §5 (private ILBs cannot receive Internet ingress; the VIP was unreferenced after the v4.6 F5 route deletion).
...
The EW NVA SNATs **only** traffic toward spokes (`10.4.0.0/14`, §6.5). Two path families deliberately remain **un-SNAT'd**:
...
> **Design invariant**: any new flow added to this architecture must either (a) be SNAT'd by the NVA, or (b) have **both** directions routed through `ILB-EW-Outbound`. A flow whose return leg bypasses the ILB (system route, different ILB, or direct peering) will be dropped by the NVA's state table. Validate with the §17 Phase 7 tests after any routing change.

> **v5.1 extension**: §6 above describes a **single NVA group per tier** — the default, reproduced exactly. For ordered **multi-group chains** see §20, **chained Azure Firewall** on the same tiers see §21, and **VMSS-based appliance groups** see §22. Each chain segment must independently satisfy the invariant above — both directions through the same group ILB, or SNAT at the group.
...
| To-NVA-Internal-Direct — **one per NVA internal subnet** (EW + NS, incl. §20 chain segments) | `10.0.5.0/24`, `10.0.3.0/24`, (`10.0.9.0/24`, …) | Virtual Network | — |

**BGP Propagation**: ❌ Disabled

> **v5.2 (F1) — why three /19s instead of the /16**: the spoke's peering system routes equal the hub VNet's **declared prefixes** (the three /19s since v5.0 C-1). A `/16` UDR is less specific and loses the LPM tie-break — every spoke→hub flow silently bypassed the EW NVA (a v5.0 regression: with a /16 hub VNet the prefixes matched and the UDR won). Equal-prefix /19 UDRs restore inspection; `To-Hub-Plan /16` is retained purely as the inspected black-hole for unassigned hub space (§3.0). `To-Bastion-Direct` keeps Bastion→VM session returns off the inspection path (Bastion traffic arrives via the VNet directly — a steered return would be one-sided and dropped). `To-OtherSpokes` may stay a summary: spokes never peer with each other, so no competing system route exists.
...
>
> **When the tier is chained (§20)**: this table instead carries exactly one route — `To-NextChainHop 0.0.0.0/0 → <next segment hop>` — and each later segment's own subnet RT continues the cascade (`RT-NS-Internal-i`); the **last** segment returns to the empty table above (egress via the external NICs).
...
> **v5.2 (F2) — role in chained tiers**: with ≥ 2 chained EW groups this table describes **group 1's client-facing subnet only** (returns/deliveries toward clients + the BGP on-prem return path). Group 1's *forward* leg leaves from `Subnet-EW-Forward` (`RT-EW-Forward`: per-spoke + platform prefixes → hop 2), and segments 2…n use `RT-EW-Internal-i` — see §20.5 for the full cascade tables.
...
| RT-NS-Internal-i / RT-FW-Internal-i (chain segments, §20.5) | ❌ Disabled (last EW/single segment: ✅ Enabled) | Mid segments use explicit cascade routes only; the LAST East-West/single segment needs gateway routes for on-prem deliveries. |
| RT-EW-Forward / RT-FW-Forward (§20.5) | ❌ Disabled | Forward-steering only — explicit cascade routes. |
| RT-AzureFirewallSubnet (when chained mid-slot, §21) | ❌ Disabled | Explicit cascade routes to the next chain hop. |
...
| RT-NVA-Mgmt | ❌ Disabled | Management plane follows the same inspected paths as platform workloads (§7.7). |
| RT-Management | ❌ Disabled | Identical routes to RT-Platform-Workloads; separate object for blast-radius isolation (§7.8). |
...
> **v5.2 (F4)**: the NSG applies to **every** NS internal subnet (base + §20 chain segments). `AllowFromChainSegments` legalises the fabric-routed hop-to-hop forward legs (each hop is a NEW inbound flow at the next segment; returns ride NSG flow state and need no rule). Without it the custom `DenyAllInbound 4096` silently killed chained traffic — the EW tier only escaped via its broad `AllowFromPlatform` rule.
...
Applies to **all** East-West internal subnets: `Subnet-EW-Internal` (+ `Subnet-EW-Internal-i` chain segments and `Subnet-EW-Forward` when chained — §20).
...
| 125 | AllowFromAzureFirewall *(when chained into E-W — §21)* | Inbound | `10.0.0.128/26` | EW internal subnet(s) | Any | Any | Allow |
...
> **v5.2 fix — Key Vault PE reachability (rule 115, F5)**: `Subnet-KeyVault` hosts private endpoints, and with `privateEndpointNetworkPolicies = Enabled` this NSG is enforced on them — yet no prior rule allowed 443 to `10.0.35.0/24`, so **every** Key Vault PE was unreachable. Spoke clients arrive post-SNAT from the East-West inspector (§6.5 v5.2 scope), hub/management clients intra-VNet, and on-prem arrives post-SNAT via the EW path (`RT-GatewaySubnet` routes Shared /19 through it).
...
For every `privatelink.*` zone in §11.5.1, on-prem DNS servers must have a conditional forwarder:
...
|---|---|
| Spoke workloads | UDR `0.0.0.0/0 → ILB-NS-Outbound` (§7.1); NS NVA owns public IPs |
| Platform/Shared Services/Management | UDR `0.0.0.0/0 → ILB-NS-Outbound` (§7.6–§7.8) |
| NS NVA external NICs | Instance-level Public IPs (§6) |
...
Every invariant in this document that a workload team could accidentally violate is enforced as **Azure Policy at the management-group scope** (Corp/Online inherit them — §1.2.2), complementing the Terraform plan gates in the fact-check report's Section L. Start from the [ALZ policy assignments](https://aka.ms/alz/policies) baseline, then layer these design-specific guardrails:

| # | Guardrail (effect) | Enforces | Scope |
|---|---|---|---|
| G-1 | **Deny** VNet creation whose address space is outside the IPAM-reserved CIDR for that subscription | §2/§4.4 — no rogue address space, no overlaps | Corp + Online |
| G-2 | **Deny** subnets without an NSG (exempt: GatewaySubnet, RouteServerSubnet, AzureFirewall*, DNS-resolver delegated subnets) | §9, §12.1–§12.5 | All LZ MGs |
| G-3 | **Deny/Modify** route tables on GatewaySubnet containing `0.0.0.0/0`, or with BGP propagation disabled | §7.0, §12.1 | Connectivity |
| G-4 | **Deny** spoke route tables with BGP propagation **enabled**, and **DeployIfNotExists** the `RT-Spoke-Workloads` route set + association on every spoke subnet | §7.1, §8, §2.1 rule 2 | Corp + Online |
| G-5 | **Deny** VNet peerings missing the §4.3 flag combination (spoke side: `useRemoteGateways=true`, `allowForwardedTraffic=true`); **Deny** spoke↔spoke peerings entirely | §4.3, §4.3.3 | Corp + Online |
| G-6 | **Deny** subnets with `defaultOutboundAccess` enabled | §12.6 | All LZ MGs |
| G-7 | **Deny** public IP addresses in **Corp** landing zones (Online exempted for AppGW/LB frontends) | §1.2.2 Corp definition | Corp |
| G-8 | **Deny** DNS forwarding-ruleset rules named `.` (wildcard), and **Deny** Private DNS zone creation/links outside the Connectivity subscription | §3.2.1 wildcard caveat, §11.5 hub-only linkage | Tenant root / Connectivity |
| G-9 | **DeployIfNotExists** DDoS Network Protection plan association on every VNet | §1.1 DDoS row | All MGs |
| G-10 | **DeployIfNotExists** `privateEndpointNetworkPolicies = Enabled` on PE subnets | §11.4 | Corp + Online |
| G-11 | **Audit** NSG rules denying `AzurePlatformDNS` / `AzurePlatformIMDS` / `AzurePlatformLKM` service tags (legitimate only with documented justification) | §9 platform-VIP hygiene | All LZ MGs |
| G-12 | **Audit** any subnet delegated to `Microsoft.Network/dnsResolvers` carrying an NSG | §3.2/§12.5 conservative stance | Connectivity |
...
│   │   └── 10.0.96.0–10.0.255.255 (plan-reserved, NOT VNet-assigned — §3.0)
│   ├── 10.1.0.0/16 + 10.2.0.0/15 (Platform growth — reserved, §2)
│   └── 10.4.0.0/14 (Application Landing Zones)
│       ├── 10.4.0.0/15 (Production — Medium /22 bottom-up, Large /20 top-down, §4.1.1)
│       ├── 10.6.0.0/16 (Development — Small /24)
│       └── 10.7.0.0/16 (Test/QA — Small /24)
└── 10.8.0.0/13 (REGION 2 — reserved, identical internal template, §2.1)
```
...
|---------|-----------|-------------|
| **Declared on the Hub VNet** (three /19 prefixes — v5.0, §3.0) | 24,576 | 37.5% of the /16 plan block; **100% of VNet space is sectioned** |
| Actually subnetted today (sum of all defined subnets, §3) | 3,936 | 6.0% of the /16 / 16.0% of the VNet |
| Plan-reserved, **not VNet-assigned** (`10.0.96.0`–`10.0.255.255`) | 40,960 | 62.5% — added to the VNet as future /19 prefixes (+ peering resync, §2.2) or used for separate platform VNets |
...
### 15.3 Application LZ Spoke Capacity (with §4.1.1 T-shirt mixing)
...
|---|---|---|---|
| **VNet peerings per VNet** (the Hub) | 500 | 1 per spoke (+1 if a Region-2 hub peers in) | **Binding constraint**: max ~499 concurrent spokes per regional hub — *below* the address plan's 640-spoke theoretical max. Track in IPAM; at ~400 spokes plan either a second hub VNet (from platform-growth space) or the §1.2.1 Virtual WAN re-evaluation. |
| **ER private-peering prefixes advertised Azure → on-prem** | 1,000 | 3 hub prefixes (v5.0, §3.0) + 1 per spoke (every spoke is single-prefix by design) | 3 + 499 = **502 ≤ 1,000** ✅ — and spoke prefixes cannot be summarised over ER (each VNet advertises its own space), so **keep spokes single-prefix**: a policy-level rule, enforced by G-1/§4.4 vending. |
...
> **Design rule derived**: the practical Region-1 ceiling is **≈ 499 spokes**, not 640. The §4 pools deliberately over-provision address space relative to this ceiling — that is correct CAF behaviour (address space is cheap to reserve, impossible to retrofit), not waste.

> **v5.1 extension**: §25 adds the per-tier NVA-subnet capacity guardrails introduced with NVA group chains (§20) — the 96 static-NIC ceiling, the 155 chained-group VIP-ladder ceiling, and the exact 251-address-per-/24 check.
...
| /22 | 1,024 (1,019 usable **only as one flat subnet**; templated spoke per §4.2 = 487 usable across its five subnets) | Standard spoke VNet |
...
- [ ] Confirm the §1.2.1 topology decision record still holds (regions ≤ 2, branches < 30, ER primary) — re-run if any trigger fired
- [ ] Validate `<on-prem-supernet>` and all individually advertised on-prem prefixes do **not** overlap `10.0.0.0/12` (§2.2); record VPN-NAT exceptions per overlapping legacy site
- [ ] Deploy **Azure IPAM** and seed it with the §2 regional /13 template; wire the reservation API into subscription vending (§4.4)
- [ ] Create the **Connectivity subscription** under the Connectivity MG; confirm Corp/Online MG hierarchy and ALZ baseline policy assignments are in place (§1.2.2)
- [ ] Deploy the single **DDoS Network Protection plan** in the Connectivity subscription (§1.1)
- [ ] Assign the §12.7 guardrail policies (G-1…G-12) at the appropriate MG scopes
- [ ] Decide AVNM adoption: connectivity configuration for hub-and-spoke peering at scale, security-admin rules for absolute denies (§4.3 note, §12.7)
- [ ] Create region-scoped resource groups for hub resources (§2.1 rule 4); enable Network Watcher + network insights in every in-scope region
...
- [ ] Create Hub VNet with **exactly three address prefixes** — `10.0.0.0/19`, `10.0.32.0/19`, `10.0.64.0/19` (§3.0; **not** the full `/16` — CAF) — and associate the DDoS plan from Phase 0
- [ ] Create all hub subnets per Section 3 (including DNS subnets with delegation `Microsoft.Network/dnsResolvers`), each with `defaultOutboundAccess = false` (§12.6)
- [ ] Verify VNet encryption is **disabled** on the Hub VNet (DNS Private Resolver requirement)
- [ ] Deploy VPN/ExpressRoute Gateway at the SKU chosen in Section 1.1 — note the gateway will advertise **3 hub prefixes** to on-prem (§15.4)
...
- [ ] Deploy Azure Bastion with NSG-Bastion (Section 9.1) — confirm the protocol split: `Any` for `AllowBastionHostCommunication`, `AllowSshRdpOutbound`, `AllowBastionCommunication`, `AllowHttpOutbound`; `Tcp` for the remaining 4 (per the MS summary table; see §9.1 validation notes on the doc's internal conflict)
...
- [ ] Reserve the spoke CIDR via the **IPAM API in the vending pipeline** at the requested T-shirt size (§4.1.1, §4.4) — never hand-pick addresses
- [ ] Create spoke VNet from the matching template (§4.2 Medium/Large, §4.2.1 Small) as a **single-prefix** VNet (§15.4), with the DDoS plan associated and `defaultOutboundAccess = false` on every subnet
...
- [ ] Online spokes only: deploy the application's **AppGW/WAF inside the spoke** (§1.2.2, §5 inbound note) — never in the hub
...
- [ ] Configure DNS forwarding ruleset on the outbound endpoint for on-prem domains, and **link the ruleset to the Hub VNet** (spoke links are ignored because spokes use custom DNS — see §18.9)
- [ ] If a wildcard `.` forwarding rule exists in the ruleset, confirm the resolver and the ExpressRoute gateway are not constrained by the wildcard caveat in §3.2.1
...
### 19.1 Fixes from prior versions (retained in v4.7)

| Issue | Problem | Fix Applied |
|-------|---------|-------------|
| **NVA Routing Loop** | RT-EW-External and RT-EW-Internal pointed `To-Spokes` to ILB, creating infinite loop | Changed to `Virtual Network` (system routes via peering) |
| **SNAT Return Path** | Spoke UDR sent all Hub traffic (`10.0.0.0/16`) to ILB, breaking SNAT return | Added `To-NVA-Internal-Direct` route (`10.0.5.0/24` → `Virtual Network`) |
| **Private Endpoint NSG** | `privateEndpointNetworkPolicies = Disabled` prevented NSG enforcement | Changed to **Enabled** to allow NSG rules |
| **Transit ILB Architecture** | Separate Transit subnet required 3rd NIC on NVAs | Consolidated Transit VIP into EW-Internal subnet |
| **GatewaySubnet No Route Table** | On-prem traffic to spokes bypassed NVA inspection entirely | Added `RT-GatewaySubnet` with spoke/platform routes pointing to ILB-EW-Outbound (no 0.0.0.0/0 route, BGP enabled) |
| **Production Max Spokes** | Documented as 512 but /15 ÷ /22 = 128 mathematically | Corrected to 128 |
| **NSG-NS-Internal Source** | AllowFromEW used `10.0.4.0/22` which included management and unused subnets | Tightened to `10.0.4.0/23` (EW-External + EW-Internal only) |
| **RT-SharedServices Redundancy** | RT-SharedServices and RT-Platform-Workloads had identical routes but were assigned inconsistently | Consolidated into RT-Platform-Workloads; Subnet-KeyVault now uses RT-Platform-Workloads |
| **ILB-EW-Inbound Unused** | ILB-EW-Inbound (10.0.4.100) was defined but no route table referenced it | Removed from ILB table; EW-External NICs retained for management/future use |

### 19.2 Fixes applied in v4.7 (from second-opinion peer review of v4.6)

| ID | Section(s) | Issue in v4.6 | Resolution in v4.7 |
|----|-----------|---------------|--------------------|
| G1 | §3.2.2 (NEW) | DNS Resolver subnets use `RT-Platform-Workloads`, which routes `0.0.0.0/0 → NS NVA` and `<on-prem-supernet> → EW NVA`. The NVA firewalls must explicitly permit UDP/TCP 53 from the outbound endpoint, and UDP/TCP 53 inbound to `10.0.32.4` from on-prem. This dependency was implicit in v4.6, risking silent DNS resolution failures at deployment. | Added Section 3.2.2 "NVA Firewall Dependency for DNS Resolver Egress" with explicit source/destination/port mapping per flow. Added Phase 2 checklist item to ensure the NVA runbook covers it. |
| G2 | §7.2 | The `To-Spokes` and `To-SharedServices` routes on RT-NS-External pointing to the internal ILB (`10.0.3.100`) were flagged in external review as a potential sandwich routing loop. In the correctly-configured NVA OS case they never fire; in the misconfigured case they cascade stray traffic through NS internal → EW for inspection (not a loop). However, a future editor could misread them as redundant and remove them, which would bypass inspection for hairpinned traffic. | Added a v4.7 clarification note under §7.2 explaining the defensive intent, the cascade path, and explicit guidance not to remove these routes without first validating NVA OS routing behaviour. |

> **Findings that were investigated but not changed in v4.7**: the peer review's P0 claim of an on-prem return-path blackhole on `RT-EW-Internal` was **already resolved in v4.6** (BGP propagation flipped from Disabled to Enabled in §7.5). The peer review was conducted against v4.5 and did not account for the v4.6 fix. Peering-flag and Private-DNS-zone-linkage findings were also already addressed in v4.6 (§4.3 and §11.5 respectively).

### 19.3 Fixes applied in v4.6 (from v4.5 review)

| ID | Section(s) | Problem in v4.5 | Fix Applied in v4.6 |
|----|-----------|-----------------|---------------------|
| F1 | §9.1, §19 | v4.5 changelog claimed Microsoft requires protocol `Any` for 4 Bastion NSG rules and changed `Tcp` → `Any`. The MS canonical reference (`bastion-nsg.md` PowerShell sample, last updated 2025-08-19) uses `Tcp` for **all 8 rules**. The v4.5 "fix" was a regression that broadened attack surface. | Reverted `AllowBastionHostCommunication`, `AllowSshRdpOutbound`, `AllowBastionCommunication`, `AllowHttpOutbound` to protocol `Tcp`. Updated validation notes. |
| F2 | §4.3 (NEW) | Hub-spoke peering flags (`AllowGatewayTransit`, `UseRemoteGateways`, `AllowForwardedTraffic`) were never explicitly documented — only "use gateway transit" appeared in the implementation checklist. Silent-failure mode at deployment. | Added Section 4.3 with explicit flag tables for both peering directions, plus a checklist item in Phase 5. |
| F3 | §11.5 (NEW), §11.4 | Private DNS zone linkage was vague ("link to Hub VNet") with no per-service zone enumeration. Risk: PEs silently resolve to public IPs if the corresponding `privatelink.*` zone is missing. | Added Section 11.5 with per-service zone table, link topology, and on-prem conditional-forwarder configuration. Added Phase 6 to checklist. |
| F4 | §11.3 | NSG-PrivateEndpoints rule 100 used source `10.4.0.0/14`. Spoke→PE traffic transits the EW NVA and is SNAT'd to `10.0.5.0/24`. Azure NSGs evaluate post-SNAT source, so the rule never matched the intended traffic — the default `AllowVnetInBound` (priority 65000) silently covered it. | Changed source to `10.0.5.0/24` (post-SNAT EW NVA range). Renamed rule to `AllowFromEW-SNAT`. Added rationale note. |
| F5 | §7.3 | RT-NS-Internal contained `0.0.0.0/0 → ILB-NS-Inbound (10.0.2.100)`, a hairpin that would send Internet-bound traffic from the NVA's internal NIC back to its own inbound ILB. No documented scenario justified it. | Removed the route. Internal-side traffic uses system default routing; NVA OS routes Internet replies out the external NIC. |
| F6 | §9.2 | NSG-NS-External had `AllowVPN-Inbound` from `192.168.0.0/16`. Per Flow 13.3, on-prem traffic enters via GatewaySubnet → ILB-EW-Outbound — it never reaches Subnet-NS-External. The rule was dead weight. | Removed the rule. |
| F7 | §7.2, §8 | RT-NS-External had BGP propagation **enabled**, but the NS NVA external NICs face Internet via Public IPs and never originate or terminate on-prem traffic. Receiving on-prem prefixes here had no benefit and risked routing artifacts. | Flipped to **disabled**. Section 8 guardrails table updated. |
| F8 | §7.5, §8 | RT-EW-Internal had BGP propagation **disabled**. The EW NVA's internal NIC participates in bidirectional on-prem ↔ Azure flows (Flow 13.3 return path). Without propagation, on-prem prefixes were absent from the effective routes and the NVA had no path back to on-premises. | Flipped to **enabled**. Explicit UDRs for `To-Spokes` and `To-Hub` (Virtual Network next-hop) override BGP-learned routes for those ranges via LPM. |
| F9 | §7.1, §7.6 | On-prem prefix was hard-coded to `192.168.0.0/16` in `To-OnPremises` UDRs. Since BGP propagation is disabled on these route tables, any on-prem prefix outside that range would have **no route** in spokes/platform — traffic would fall through to the `0.0.0.0/0 → NS NVA` route and try to egress to Internet. | Replaced with `<on-prem-supernet>` placeholder and added explicit parameterisation guidance for non-contiguous on-prem ranges. |
| F10 | §1.1 (NEW) | No SKU was specified for any platform service except ILBs. Behaviour and feature support vary materially by SKU. | Added Section 1.1 SKU Assumptions with rationale per service. |
| F11 | §3.2.1, §12.5 | DNS Private Resolver compatibility constraints (ExpressRoute FastPath, Azure Lighthouse, VNet encryption) were not documented. The categorical "NSGs are not supported on DNS Resolver delegated subnets" claim was more restrictive than current MS docs. | Added explicit incompatibility notes (FastPath, Lighthouse, encryption). Softened the NSG claim to align with current MS docs while keeping the conservative default of omitting NSGs. |
| F12 | §11.4 | `privateEndpointNetworkPolicies` was documented only as `Enabled`/`Disabled`. Current ARM/portal supports four values: `Disabled`, `NetworkSecurityGroupEnabled`, `RouteTableEnabled`, `Enabled`. | Documented all four valid values; justified `Enabled` as this design's choice. |
| F13 | §4.2 | The /22 spoke template uses 512 of 1,024 addresses. The unused upper /23 was implicit. | Added explicit reservation note for the upper /23 of each spoke. |

### 19.4 Fixes applied in v4.9 (live-doc fact-check of v4.7, 2026-06-11)

> Every claim below was re-verified against **live Microsoft Learn pages fetched on 2026-06-11**, not training-data memory. The file circulated as `v4_8.md` was byte-identical to `v4_7.md` (same MD5); v4.9 supersedes both.

**P1 — functional or factual errors:**

| ID | Section(s) | Problem (as in v4.7) | Fix Applied | Reference |
|----|-----------|----------------------|-------------|-----------|
| F-A | §9.1, §17 | All 8 Bastion NSG rules set to `Tcp` (the v4.6 F1 "fix"). The **current** `bastion-nsg` page now contains a normative summary table requiring protocol `*` (Any) for 4 rules, while its PowerShell sample on the same page still shows `Tcp` for all 8 — the page self-contradicts. v4.6's all-Tcp was an over-correction; v4.5's `Any` was defensible. | Followed the normative summary table: `Any` for `AllowBastionHostCommunication`, `AllowSshRdpOutbound`, `AllowBastionCommunication`, `AllowHttpOutbound`; `Tcp` for the rest. Documented the doc conflict and tiebreaker policy in §9.1. | [bastion-nsg](https://learn.microsoft.com/en-us/azure/bastion/bastion-nsg) |
| F-B | §9.6–§9.9 | Custom `DenyAllInbound 4096` on spoke/shared NSGs blocked **all Bastion sessions** — the doc-required allow of 22/3389 from `AzureBastionSubnet` on target-VM subnets was missing, and the default `AllowVnetInBound` (65000) never fires below a 4096 deny. | Added rule 140 `AllowBastionInbound` (source `10.0.1.0/26`, ports 22,3389/TCP) to NSG-SharedServices, NSG-Spoke-Web/App/Data. | [bastion-nsg](https://learn.microsoft.com/en-us/azure/bastion/bastion-nsg) |
| F-C | §9.6 | `AllowAD` rule (389,636,88,445 **TCP only**, source `10.0.0.0/12`) functionally **broke AD**: missing UDP 53/88/123/389/464, TCP 53/135/464/3268/3269 and the dynamic RPC range; on-prem sources (replication, logons) excluded entirely. | Rewrote as AllowAD-TCP + AllowAD-UDP with the full documented port set and `<on-prem-supernet>` added to sources. | [Config firewall for AD domains](https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/config-firewall-for-ad-domains-and-trusts) |
| F-D | §11.3 | On-prem → Hub PE was **denied** despite §11.5.3 promising it: NSG-PrivateEndpoints allowed only `10.0.5.0/24` + `10.0.32.0/19`, so rule 4096 dropped all on-prem sources. | Added rule 120 `AllowFromOnPrem` (443/TCP). Documented that the path is deliberately direct/uninspected and must **not** be forced through the EW NVA without extending SNAT scope. | [Private Endpoint NSG](https://learn.microsoft.com/en-us/azure/private-link/disable-private-endpoint-network-policy) |
| F-E | §5, §6.4 | `ILB-NS-Inbound (10.0.2.100)` still listed though unreferenced since v4.6 F5 — and a **private-frontend ILB cannot receive Internet ingress** at all. | Removed from §5 and the NS loopback list; documented the real inbound patterns (NVA instance PIPs / public Standard LB / AppGW–Front Door). VIP reserved, not reassigned. | [HA-NVA guide](https://learn.microsoft.com/en-us/azure/architecture/networking/guide/network-virtual-appliance-high-availability) |
| F-F | §4.3.1, §4.3.2 | The two `AllowForwardedTraffic` rationale cells were **swapped** — each side's flag governs traffic *that* VNet accepts *from* the remote VNet; the spoke-side flag is the critical one for un-SNAT'd NVA return legs. Values were both `true`, so functionally harmless, but the documentation taught the wrong mental model. | Rewrote both rationale cells with correct semantics and a v4.9 note. | [VNet peering](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-peering-overview) |
| F-G | §12.3 | "UDR only for forced tunneling scenarios" on AzureFirewallSubnet — wrong: a `0.0.0.0/0 → Internet` override UDR is **mandatory** whenever on-prem advertises a default route via BGP. | Documented both sanctioned UDR uses. | [Azure Firewall known issues](https://learn.microsoft.com/en-us/azure/firewall/firewall-known-issues) |
| F-H | §3.2, §18.8 | Internal contradiction: §3.2 bold "NSGs are **not supported** on DNS Resolver delegated subnets" vs the softened (correct) §3.2.1/§12.5 wording. | Harmonized: NSGs omitted by conservative choice; current docs don't explicitly prohibit them. | [DNS Private Resolver](https://learn.microsoft.com/en-us/azure/dns/dns-private-resolver-overview) |
| F-I | §7.7, §7.8 (NEW), §8, §3.1 | `RT-NVA-Mgmt` and `RT-Management` were referenced in §3.1/§3.3 but **never defined**; `RT-PrivateEndpoints` referenced but undefined and unnecessary (PE return traffic bypasses UDRs). | Defined both route tables (mirroring RT-Platform-Workloads) + §8 rows; PE-Hub subnet RT set to "—" with rationale. | [UDR overview](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-udr-overview) |
| F-J | §18.9, §17 Phase 6, §3.2.1 | "Verify DNS forwarding ruleset is linked to **spoke** VNets" — ineffective: per the resolver's query process, VNets with custom DNS (all spokes) skip their own ruleset links; the ruleset must link to the **Hub VNet**. Also missing: the ER-gateway wildcard-rule co-location caveat. | Corrected linkage target everywhere; added the wildcard (`.`) rule prohibition for the Hub (resolver + ER gateway co-located). | [DNS Private Resolver](https://learn.microsoft.com/en-us/azure/dns/dns-private-resolver-overview), [ER gateway](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-about-virtual-network-gateways) |

**P2 — precision, completeness, hardening:**

| ID | Section(s) | Change | Reference |
|----|-----------|--------|-----------|
| F-K | §6.5 | Added RFC 6598 (`100.64.0.0/10`) to the Azure Firewall default no-SNAT set; noted `IANAPrivateRanges` keyword and that policy-managed firewalls ignore the classic `PrivateRange` property. | [SNAT private ranges](https://learn.microsoft.com/en-us/azure/firewall/snat-private-range) |
| F-L | §1.1 | Bastion port-80 rule is a base requirement for **all** SKUs (removed unsupported "Standard/Premium" qualifier); **ErGw2AZ has no FastPath capability** (FastPath needs ErGw3AZ / Ultra / ErGwScale ≥ 10). | [bastion-nsg](https://learn.microsoft.com/en-us/azure/bastion/bastion-nsg), [ER gateways](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-about-virtual-network-gateways) |
| F-M | §6.7 (NEW) | Documented **why the un-SNAT'd paths work**: the Standard LB same-instance selection property when both flow directions traverse the same LB. Added the design invariant for future flows. | [HA-NVA guide](https://learn.microsoft.com/en-us/azure/architecture/networking/guide/network-virtual-appliance-high-availability) |
| F-N | §2, §15 | Made the silent `10.1.0.0–10.3.255.255` gap explicit (reserved for platform growth, summarisable as `10.0.0.0/14`); replaced the wrong "~8,192 / ~13%" Platform utilisation (matched neither measure) with both correct views: 24,576 / 37.5% committed to /19 sections, 3,936 / 6.0% actually subnetted. | Python `ipaddress` verification |
| F-O | §12.6 (NEW) | Default outbound access retirement: VNets via API versions after 31 Mar 2026 default to private subnets. Design already compliant by construction; added explicit `defaultOutboundAccess = false` + Azure Policy recommendations. | [Default outbound access](https://learn.microsoft.com/en-us/azure/virtual-network/ip-services/default-outbound-access) |
| F-P | §6.6 | Clarified throughput column = vendor inspection figures, not NIC bandwidth (Dsv5 D4s = 12.5 Gbps platform limit). | [Dsv5 series](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/general-purpose/dsv5-series) |
| F-Q | §1 | Zero-Trust principle row now names the two deliberate inspection exceptions (intra-hub SS→PE, on-prem→PE) and their compensating controls. | — |
| F-R | §12.2 | Added verified Route Server guardrails: ASN restrictions (≠65515, reserved lists, 16-bit only), 16-peer / 4,000-route limits, dual BGP sessions + eBGP multi-hop, control-plane-not-data-plane, branch-to-branch, `no-advertise` community, DDoS. | [Route Server FAQ](https://learn.microsoft.com/en-us/azure/route-server/route-server-faq), [Secure Route Server](https://learn.microsoft.com/en-us/azure/route-server/secure-route-server) |
| F-S | §16 | /22 row clarified: 1,019 usable only as one flat subnet; templated spoke = 487 usable. | — |
| F-T | §4.2.1 (NEW) | Dev/Test /24 spokes had **no template** (the /22 template spans 512 addresses — cannot fit). Added the /24 carve with identical NSG/RT objects. | — |

---

### 19.5 Changes applied in v5.0 (CAF / ALZ alignment, 2026-06-11)

> v5.0 aligns the design with six Cloud Adoption Framework / Azure Landing Zone pages, **fetched live on 2026-06-11**: [landing-zone overview](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/landing-zone/), [design principles](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/landing-zone/design-principles), [network topology & connectivity design area](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/landing-zone/design-area/network-topology-and-connectivity), [define an Azure network topology](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/define-an-azure-network-topology), [traditional Azure networking topology](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/traditional-azure-networking-topology), and [plan for IP addressing](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/plan-for-ip-addressing).
>
> **Invariants**: zero subnet renumbering — every CIDR, route, NSG rule, ILB VIP, and flow from v4.9 is unchanged and re-verified; all twenty v4.9 fixes (F-A…F-T, §19.4) are retained. The only *structural* change is C-1.

| ID | Section(s) | Change | CAF source |
|----|-----------|--------|------------|
| C-1 | §3.0 (+ §14, §15.2, §17 Phase 1) | **Structural**: the Hub VNet no longer declares `10.0.0.0/16`. It declares exactly **three address prefixes** — `10.0.0.0/19` + `10.0.32.0/19` + `10.0.64.0/19` (24,576 addresses, 100% sectioned). `10.0.96.0–10.0.255.255` stays plan-reserved, not VNet-assigned; growth = add a 4th prefix online + per-peering resync. All 21 subnets verified ⊂ the three /19s — nothing renumbered. Side-effects assessed: gateway advertises 3 prefixes; `VirtualNetwork` tag narrows (all rules use literal CIDRs); spoke `To-Hub 10.0.0.0/16` route deliberately kept as an inspected black-hole for unassigned space. | plan-for-ip-addressing — *"Don't create large virtual networks like /16"* |
| C-2 | §2, §14, §15.1 | Master `/12` re-expressed as **two regional /13s** with identical internal templates: Region 1 = `10.0.0.0/13` (Platform /16 + growth `10.1.0.0/16`+`10.2.0.0/15` + spokes `10.4.0.0/14` — verified to tile the /13 exactly); Region 2 = `10.8.0.0/13` reserved (formerly "Future Expansion"); Region 3+ requires a new non-overlapping /12 reserved in IPAM. | plan-for-ip-addressing — non-overlap per region planned in advance |
| C-3 | §2.1 | Multi-region rules: global hub-hub peering for 2 regions; remote-spoke reachability only via explicit UDR through the **local** EW ILB; spoke RTs' BGP-off already implements the CAF cross-region inspection mitigation; >2 regions → same ER circuits or vWAN re-evaluation; region-scoped RGs + AVNM. | traditional-azure-networking-topology |
| C-4 | §2.2 | Address-space hygiene hard rules: RFC 1918 only; never-use list (`224.0.0.0/4`, `255.255.255.255/32`, `127.0.0.0/8`, `169.254.0.0/16`, `168.63.129.16/32`); pre-deployment on-prem overlap gate with **VPN-gateway NAT** as the per-site escape hatch; address-space additions are online + per-peering resync; IPv4-exhaustion levers (nonroutable spokes + Private Link). | plan-for-ip-addressing |
| C-5 | §2.3 | IPv6 stance recorded: IPv4-only by decision (resolver/Route Server constraints, /64-only IPv6 subnets, IPv4 can never be disabled); IPv6-client edge patterns documented (Front Door L7 / dual-stack NVA VMSS-Flex L4) for adoption without re-platforming. | plan-for-ip-addressing — IPv6 considerations |
| C-6 | §4.1.1 | **T-shirt sizing catalog** straight from CAF: Small `/24` (Dev/Test), Medium `/22` (Prod default, bottom-up from `10.4.0.0/22`), Large `/20` (top-down from `10.5.240.0/20`; each Large = a /20-aligned run of 4 /22 slots — two-pointer allocation, fragmentation-free, verified). AKS-CNI workloads default to Large. | plan-for-ip-addressing — IPAM T-shirt example (S=/24, M=/22, L=/20) |
| C-7 | §4.4, §17 Phase 0/5 | **Azure IPAM + subscription vending** integration: reservation API called before `terraform plan`, blocks structured region→environment→archetype, decommissioning releases slots. | plan-for-ip-addressing — IPAM tools |
| C-8 | §1.1, §17 Phase 0/1/5 | **Single Azure DDoS Network Protection plan** in the Connectivity subscription, associated to hub + all spoke VNets (also closes the §12.2 Route Server DDoS guardrail). | traditional-azure-networking-topology |
| C-9 | §5 inbound note, §1.2.2, §17 Phase 5 | **L7 inbound NVAs are never shared hub services**: Application Gateway/WAF deploys per-application **inside the owning spoke** (Online pattern); Front Door for global entry. | traditional-azure-networking-topology |
| C-10 | §1.2.1 | **Topology decision record**: traditional hub-and-spoke justified against the CAF criteria table (≤ few regions, <30 IPsec tunnels, full routing control, centralized NVAs) with explicit **vWAN re-evaluation triggers**. | define-an-azure-network-topology |
| C-11 | §1.2.2 | **Subscription / management-group mapping**: Connectivity sub = hub VNet + gateways + NVAs + Bastion + resolver + all `privatelink.*` zones + DDoS plan; Identity = DCs (hub-resident, CAF-sanctioned, with peered-VNet alternative); Management sub = PaaS estate with touchpoints in the Mgmt /19; Corp MG = all Prod/Dev/Test spokes; Online MG = public-facing spokes; policy inheritance → §12.7. | landing-zone overview + network design area |
| C-12 | §1.2.3 | **Design-principles matrix + deviation register**: subscription democratization, policy-driven governance, single control plane, application-centric model — and the one documented deviation (third-party NVAs vs Azure-native), with the pre-provisioned **Azure Firewall exit ramp** (`AzureFirewallSubnet` reserved, §6.5/§12.3 behaviour already documented) making reversal a cut-over, not a redesign. | design-principles |
| C-13 | §12.7 | **Azure Policy guardrail catalog G-1…G-12** (deny non-IPAM address space, NSG-less subnets, GatewaySubnet 0/0/BGP violations, spoke BGP-on, bad peering flags, implicit outbound, Corp public IPs, wildcard DNS rules, zones outside Connectivity; DINE DDoS association + PE policies; audits for AzurePlatform\* denies and resolver-subnet NSGs) + AVNM **security admin rules** for absolute denies. Layered on the ALZ baseline ([aka.ms/alz/policies](https://aka.ms/alz/policies)). | design-principles — policy-driven governance |
| C-14 | §15.3, §15.4 | Capacity reframed for T-shirt mixing (128 M pure / 32 L pure / 4 M-slots per L) and **platform scale guardrails**: 500-peerings-per-VNet is the **binding constraint** (~499 spokes < the 640-spoke address-plan max — plan a 2nd hub or vWAN at ~400); ER private-peering budget 3 hub + 499 spoke prefixes = 502 ≤ 1,000, contingent on the **single-prefix-spoke rule** (enforced by G-1/vending). | traditional-azure-networking-topology — limits |
| C-15 | §17 Phase 0 | New **Phase 0 — CAF governance prerequisites**: topology-DR recheck, overlap validation, IPAM deploy+seed, Connectivity sub + MG/policy baseline, DDoS plan, G-1…G-12 assignment, AVNM decision, region RGs + Network Watcher insights. | all six pages |

### 19.6 Changes applied in v5.1 (Network Design Addendum integration, 2026-06-11)

> v5.1 integrates the standalone *Network Design v5.1 Addendum* into this single document as §20–§25 and records the companion tool **AzIP-Ranger**. **Zero changes to the v5.0 core (§1–§19)**: every CIDR, route, NSG rule, ILB VIP, SNAT rule, and traffic flow is unchanged, and the single-group design is reproduced exactly. All v5.1 additions are opt-in extensions of existing sections, verified against the HA-NVA guide, Azure Firewall SNAT, Gateway Load Balancer, CAF resource naming/abbreviations, and Azure subscription-limits pages (§26).

| ID | Section(s) | Change | Source |
|----|-----------|--------|--------|
| D-1 | §20 (extends §5–§6) | **NVA Group Chains**: each tier holds an ordered list of NVA groups chained by sort order; every UDR targets only the entry group's hop; chain hops use the VIP ladder (`.100, .101, .102 …`); every group SNATs to its internal NICs so each chain segment satisfies the §6.7 symmetry invariant independently. Precise per-/24 address law (`.4–.99` statics, `.100–.254` VIP ladder, remainder VMSS) with an exact `need ≤ 251` capacity check. Standalone (explicit-proxy) groups defined. | HA-NVA guide; plan-for-ip-addressing |
| D-2 | §21 (extends §1.2.3 / §6.5 / §12.3) | **Chained Azure Firewall**: the reserved `AzureFirewallSubnet` exit ramp promoted to four deployable patterns (chained into N-S, chained into E-W, chained on the single tier, or Azure-Firewall-only); the firewall may occupy any chain slot; SNAT behaviour specified per pattern; one `AzureFirewallSubnet` per VNet and `RT-AzureFirewallSubnet` with BGP propagation disabled retained. | Azure Firewall SNAT; HA-NVA guide |
| D-3 | §22 (extends §6.6) | **VMSS-based NVAs**: VMSS Flexible, zone-spread, behind the group's Standard ILB (HA ports); instance NICs dynamic (capacity reserved, never specific addresses); the VIP is the only stable address; scale floor ≥ 2; image contract for backend-pool join + VIP loopback + internal-NIC SNAT. | HA-NVA guide; subscription limits |
| D-4 | §23 (extends §4) | **Catalog-driven spokes & CAF naming**: spokes become a free-form `{name, environment, size}` catalog (environments are labels, not pools); Reference mode allocates by size into the unchanged v5.0 pools, Auto mode creates one right-sized pool per environment; full CAF naming convention table (`vnet-`, `snet-`, `nsg-`, `rt-`, `nva-`, `vmss-`, `lbi-`); all selectable values in `web/config.js`. | CAF resource naming/abbreviations |
| D-5 | §24 (extends §1.1 / §2.1) | **Operational gating rules**: no on-prem network ⇒ no hybrid connectivity (GatewaySubnet not deployed); ER + VPN ⇒ ER primary / VPN backup (matches §1.1); the Region-2 reservation is kept unallocated so a second region deploys the identical template without renumbering. | traditional-azure-networking-topology |
| D-6 | §25 (extends §15.4) | **Updated capacity guardrails**: per-tier static-NIC ceiling **96** (`.4–.99`); chained groups per tier **155** (VIP ladder `.100–.254`); tier-subnet total (statics + VIPs + VMSS max) **251** exact check; static management NICs across all tiers **251** (`Subnet-NVA-Management` /24 — 2 × 96 always fits). | subscription limits; plan-for-ip-addressing |


### 19.7 Fixes applied in v5.2 (deep-scan remediation, 2026-06-12)

> A full validation of generated designs (`Test/VALIDATION-REPORT.md`) against **live Microsoft Learn** (route-selection rules, Bastion NSG, GWLB, AVNM, networking limits — fetched 2026-06-12) found the issues below. Root cause is attributed per item: **[DOC]** = defect originated in this document (the AzIP-Ranger engine reproduced it faithfully); **[TOOL]** = engine/template gap not present in the document. All fixes are implemented in both the document and the engine, and locked in by `tests/run.js` (81 assertions incl. an effective-route LPM simulator).

| ID | Sev | Origin | Problem | Fix |
|----|-----|--------|---------|-----|
| F1 | Critical | **[DOC]** (v5.0 C-1 regression + /14 summaries since v4.x) | Azure route selection is LPM-first across all sources; UDR priority only ties at identical prefixes. Peering routes (hub 3×/19; spoke /22–/24) beat `To-Hub /16` / `To-Spokes /14`: spoke↔hub and platform→spoke bypassed the EW tier; **on-prem↔spoke broke** (asymmetric stateful drop); `RT-AzureFirewallSubnet`'s spoke routes skipped the post-firewall hop; spoke→hub-PE/KV was denied (un-SNAT'd sources). | §7.0–§7.8 rewritten: exact hub /19s (+ /16 black-hole) in spoke RTs; one exact-prefix route per spoke in every hub-side steering table; `To-Bastion-Direct` + per-NVA-subnet direct-return exemptions; UDR-budget guardrail (400/1,000-AVNM, §15.4/§25). |
| F1b | High | **[DOC]** | `RT-NS-Internal To-Spokes → ILB-EW` steered NS→spoke deliveries/returns into the EW NVA as one-sided flows (latent — the summary prefix was inert pre-v5.2). | Route removed; empty RT kept to pin BGP-off (§7.3). Symmetry now via spoke `To-NVA-Internal-Direct` routes. |
| F2 | Critical | **[DOC]** (v5.1 §20.1/§21) | Same-subnet chain forwarding ("appliance forwards to next VIP") has no fabric mechanism — Azure ignores guest-OS next hops toward same-subnet VIPs; hops 2+ never saw traffic, mid-chain Azure Firewall unreachable. | §20 rewritten as a **fabric-routed UDR cascade**: dedicated segment subnets + per-subnet RTs (§20.5), forward subnet for EW/single group 1, firewall slots fabric-routed (§21), WARN at depth > 3 with GWLB pointer (§20.3). |
| F3 | High | **[TOOL]** (doc §11.2 underspecified) | Spoke PE NSGs reused the hub rule set; intra-spoke (un-SNAT'd, system-routed) clients were denied — every spoke's PEs unreachable from their own workloads. | `AllowFromOwnSpoke` (prio 90) per spoke instantiation (§11.3). |
| F4 | Medium | **[DOC]** §9.3 + **[TOOL]** | No intra-tier allow for chain hop-to-hop legs on the NS internal NSG (EW escaped via its broad platform rule). | `AllowFromChainSegments` (prio 115) on chained tiers; NSGs apply to all segment subnets (§9.3/§9.5). |
| F5 | Medium | **[DOC]** §9.6 | No rule allowed 443 to `Subnet-KeyVault` — with PE network policies Enabled, all Key Vault PEs were dead. | `AllowKeyVaultPE` (prio 115) with post-SNAT + intra-hub sources (§9.6); §6.5 SNAT scope extended to PE-hosting subnets. |
| F6 | Low | mixed | Dead `data→app 1433` rule (§9.8); NSG-Management source wider than the declared VNet; unexplained "+1" in the peering guardrail; CSV export omitted reserved rows; chain-depth and UDR-budget signals missing. | All corrected in doc + engine; guardrails and exports updated. |

---

## 20. NVA Group Chains (extends v5.0 §5–§6)
...
| Segment subnets | **Each chained group i ≥ 2 owns a dedicated /24 internal subnet** (`Subnet-<tier>-Internal-i`, allocated from `10.0.9.0/24` upward — §3.1) with its VIP at that subnet's `.100` (`lbi-<tier>-<group>` per CAF). A chained East-West/single tier adds **one** `Subnet-EW-Forward`/`Subnet-FW-Forward` hosting group 1's forward NICs. Standalone groups stay in group 1's subnet on the VIP ladder (`.101+`). |
| Forwarding contract | **Segment i → i+1 steering is a per-subnet UDR**, not appliance configuration: NS segments carry `0.0.0.0/0 → <next hop>`; EW/single segments carry the lateral prefix set (one exact route per spoke + Shared /19 + Management /19 + Hub-PE /24 + on-prem) → `<next hop>` (§20.5). The **last** group egresses normally (NS: external NICs → Internet; EW: Virtual Network to destination, BGP on for on-prem). **Why**: Azure forwards on the destination IP against the subnet's effective routes — a guest-OS next hop pointing at a VIP in the *same* subnet is **not honored** ([route selection](https://learn.microsoft.com/azure/virtual-network/virtual-networks-udr-overview#how-azure-selects-routes-for-traffic-routing)), and Microsoft's UDR guidance requires an NVA to sit in a **different subnet** than the resources routed through it. The v5.1 same-subnet contract was unimplementable (F2): hops 2+ never received traffic. |
| Symmetry | **Every group SNATs to the NIC it forwards from.** Each chain segment then satisfies the v5.0 §6.7 invariant independently (both directions of each segment traverse the same ILB/instance), so returns retrace the chain hop-by-hop; spoke route tables carry one `To-NVA-Internal-Direct` route per NVA internal subnet (§7.1). A group that does not SNAT breaks its segment's state table — hard requirement, identical in spirit to §6.5. |

### 20.2 Address law per segment subnet (precise, enforced — v5.2)

Usable host range of a /24: `.4 – .254` (Azure reserves `.0–.3` and `.255`; 251 usable — v5.0 §16). The law applies **per subnet**:
...
### 20.3 When NOT to chain — Microsoft-aligned decision record
...
3. **Azure Firewall + NVA** is already covered as a 2-element chain (§21) — don't model the firewall as an NVA group.

### 20.4 Standalone groups (explicit-proxy pattern)
...
### 20.5 Chain-segment route tables (v5.2 — normative)

For a tier with chained elements `E1 … En` (NVA groups and, optionally, one Azure Firewall slot — §21):

| Subnet | Route table | Routes | BGP |
|---|---|---|---|
| `Subnet-NS-Internal` (E1, North-South) | `RT-NS-Internal` | `0.0.0.0/0 → hop(E2)` — or **no routes** when n = 1 | ❌ |
| `Subnet-NS-Internal-i` (mid segments) | `RT-NS-Internal-i` | `0.0.0.0/0 → hop(E[i+1])` | ❌ |
| `Subnet-NS-Internal-n` (last) | `RT-NS-Internal-n` | *(empty — egress via external NICs)* | ❌ |
| `Subnet-EW-Internal` (E1 client side, East-West) | `RT-EW-Internal` | unchanged v5.0 anchor: `To-Spokes (VN)`, `To-Hub (VN)` | ✅ |
| `Subnet-EW-Forward` (E1 forward side) | `RT-EW-Forward` | lateral set → `hop(E2)`: one exact route per spoke + Shared `/19` + Management `/19` + Hub-PE `/24` + on-prem | ❌ |
| `Subnet-EW-Internal-i` (mid segments) | `RT-EW-Internal-i` | lateral set → `hop(E[i+1])` | ❌ |
| `Subnet-EW-Internal-n` (last) | `RT-EW-Internal-n` | `To-Spokes (VN)`, `To-Hub (VN)` — delivers via peering | ✅ |
| `AzureFirewallSubnet` (firewall slot, non-last) | `RT-AzureFirewallSubnet` | NS chain: `0.0.0.0/0 → hop(next)`; EW/single chain: lateral set (+ `0.0.0.0/0` on the single tier) → `hop(next)` | ❌ |

Design rationale, mirrored from the F2 analysis: only group 1 both **forwards** spoke-destined traffic (to E2) and **delivers** toward spoke clients (returns) — those two intents collide on one subnet, hence the dedicated forward subnet on the EW/single tier. Mid segments never deliver to clients; the last segment never forwards — both are single-subnet. Returns between segments target NVA NIC addresses (hub-internal) and ride system routes plus NSG flow state; returns to workloads ride the spoke RTs' `To-NVA-Internal-Direct` exemptions (§7.1).

---

## 21. Chained Azure Firewall (extends v5.0 §1.2.3 / §6.5)
...
## 22. VMSS-based NVAs (extends v5.0 §6.6)
...
| Image contract | The vendor image must join the ILB backend pool on scale-out, configure the VIP loopback (§6.4), and SNAT via its internal NIC. Validate §17 Phase-7 symmetry tests after every scale event. |
| Management | VMSS management NICs are dynamic in `Subnet-NVA-Management`; the NSG attaches via the scale-set NIC profile. |

---

## 23. Catalog-driven spokes & CAF naming (extends v5.0 §4)

### 23.1 Free-form spoke catalog
Spokes are a list of `{ name, environment, size }` — environments are **labels, not pools**. Sizes follow the §4.1.1 T-shirt catalog (S /24 · M /22 · L /20, extensible via `config.js`).
...
### 23.2 Naming convention ([CAF abbreviations](https://learn.microsoft.com/azure/cloud-adoption-framework/ready/azure-best-practices/resource-abbreviations))
...
## 24. Operational gating rules (tool behaviour)
...
3. ER + VPN together ⇒ ER is the primary (solid), VPN the backup (dashed) — matching v5.0 §1.1's "VPN is backup only".
...
## 25. Updated capacity guardrails (extends v5.0 §15.4)
...
## 26. Microsoft Documentation References
