...
- **Azure Policy pack**: emit the Section 12.7 guardrail catalog (G-1…G-12) as ready-to-assign policy definition JSON.
...
- **Active Region-2 generation**: turn the reserved second /13 into a fully generated mirror region with hub-to-hub global peering and cross-region inspection routes (Section 2.1).
- **IPv6 dual-stack planning**: optional /64 overlays per subnet, mirrored NSG rules, Front Door / dual-stack edge patterns (Section 2.3).
- **On-prem overlap helper**: when an on-prem site overlaps the Azure supernet, propose the VPN-gateway NAT plan for that site instead of just erroring (Section 2.2).
